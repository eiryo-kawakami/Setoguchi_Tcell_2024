			...
			while line:
				...
				if float(itemList[7]) < FDR_threshold:
					...
					if float(itemList[6]) > 0:
						fo.write('n-'+itemList[0].replace('\"','')+' v-'+str(-math.log10(float(itemList[7]))*100)+' o-0.6 c-#DA6272\n')
					else:
						fo.write('n-'+itemList[0].replace('\"','')+' v-'+str(-math.log10(float(itemList[7]))*100)+' o-0.6 c-#42AAC7\n')
				line = fi.readline()
